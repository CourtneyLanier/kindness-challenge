...
function isSlackSignatureValid({ signingSecret, body, timestamp, signature }) {
  ...
  if (Math.abs(now - Number(timestamp)) > 60 * 5) return false; // 5 min window
...
/* ---------- helpers to read existing install from Blobs via HTTP ---------- */
...
exports.handler = async (event) => {
  ...
  // Load existing config (prefill modal)
  const existing = await fetchInstall(team_id);
  const goal     = existing?.goal ?? 100;
  const start    = existing?.start ? new Date(existing.start * 1000).toISOString().slice(0,10) : '';
  const end      = existing?.end   ? new Date(existing.end   * 1000).toISOString().slice(0,10) : '';
  const channel  = existing?.channel_id || ''; // they can paste name or ID here; we’ll resolve on save
  ...
  const view = {
    ...
    private_metadata: JSON.stringify({ team_id }), // we’ll read this on submission
    blocks: [
      {
        type: 'input',
  ...
  // Open modal using your app’s bot token (env for your workspace; others can still open modal because command is tied to the app)
  try {
